...
def extract_clean_sequence(ibam="input.bam",reference="hg38.fa",outpath="raw_reads",statistic="statistic.txt",metric="metrics.txt",leftS=15,leftE=118,trapL=37):
    ...
    for record in ref_reader:
        ...
        indel_count = 0
        mismatch_count = 0
        ## cheak if the output path exists
        ...
        with open(out_file,"w+") as f:
            ...
            for read in bam_file.fetch(record.name):
                ...
                if len(search_result) > 0:
                    clean_count += 1
                    out_sequence = search_result[0][(leftE-leftS):-4]
                    control_sequence = ref_sequence[leftE:(leftE+trapL)]
    ...
    with open(f"{outpath}/{statistic}","w+") as s:
    ...
    ## write metrics file
    with open(f"{outpath}/{metric}","w+") as m:
...
extract_clean_sequence(ibam=snakemake.input['bam'],reference=snakemake.config['ref'],outpath=snakemake.output[0],leftS=snakemake.config['ExtractReads']['leftStart'],leftE=snakemake.config['ExtractReads']['leftEnd'],trapL=snakemake.config['ExtractReads']['trapLength'])